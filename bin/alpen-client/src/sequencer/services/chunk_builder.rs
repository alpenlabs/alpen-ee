...
pub(crate) async fn start_chunk_builder_service<P, D, S, CS, BS, ES>(
    genesis: BlockNumHash,
    chunk_storage: Arc<CS>,
    batch_storage: Arc<BS>,
    block_storage: Arc<ES>,
    sealing_policy: S,
    sealing_data_provider: Arc<D>,
    ...
    ES: ExecBlockStorage + 'static,
{
    ...
    let state = init_chunk_builder_state::<P>(chunk_storage.as_ref(), genesis)
        .await
    ...
    ServiceBuilder::<
        ChunkBuilderService<P, S, D, CS, BS, ES>,
