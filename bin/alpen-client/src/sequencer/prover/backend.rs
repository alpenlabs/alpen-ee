...
//! backend (`--prover-backend native`, the default, or `sp1`), builds every
//! configured `--prover-program` candidate into a launched [`ProverProgram`],
//! keyed by the `AlpenSpecId` the candidate declared, and hard-fails unless
//! at least one resident candidate's derived account predicate key matches
//! the OL's expected `update_vk` right now. Routing a given batch to the
//! right resident program (by that batch's own governing spec version) is
...
/// Picks a prover backend, builds every resident `--prover-program`
/// candidate into launched provers, and hard-fails unless at least one
...
/// Builds every configured `--prover-program` candidate, keyed by the
...
/// built (not just whichever is active right now), since
/// [`PaasBatchProver`] routes each batch's proof request by that batch's own
/// governing spec version, not by whatever's active at this moment.
    ...
    match backend {
        ProverBackendConfig::Native { programs } => {
            ...
            for program in &programs {
                ...
                if provers
                    .insert(program.spec_version, EeProvers { chunk, account })
                    .is_some()
                {
                    return Err(duplicate_spec_version_err(program.spec_version));
                }
            }
        }
        #[cfg(feature = "sp1")]
...
/// Error for when none of the resident `--prover-program` candidates'
...
fn no_matching_candidate_err(
    ...
    eyre::eyre!(
        "none of the {candidate_count} resident --prover-program candidate(s) match \
         OL's expected account update_vk {ol_account_update_vk:?}"
    )
}

/// Error for when two `--prover-program` candidates declare the same spec
/// version.
fn duplicate_spec_version_err(spec_version: AlpenSpecId) -> eyre::Error {
    eyre::eyre!(
        "multiple --prover-program candidates declare spec version {spec_version}; \
         each resident version may have exactly one candidate"
    )
}
