...
//! between the two builds. Both guests also embed the canonical `AlpenParams` (genesis,
...
//! a stable location that survives `cargo clean` and that `lib.rs` and CI reference
//! directly.
//!
...
//!   the guests; unset means "skip, no ELF produced." Must be absolute: build scripts run with CWD
...
fn chunk_predicate_condition() -> Vec<u8> {
    let elf_path = Path::new(GENERATED_DIR).join(format!("{ALPEN_CHUNK}.elf"));
...
/// `generated/alpen_params.rs`: the full JSON embedded verbatim as a real
/// string constant (decoded by each guest at startup via `serde_json`),
/// alongside a compact one-line summary as a doc comment for a quick,
/// at-a-glance record of what got baked in.
///
...
fn write_alpen_params_const(path: &Path) {
    ...
    let content = format!(
        "// Generated by `build.rs` — do not edit.\n\
         // Source: {}\n\
         // {json_oneline}\n\
         pub const ALPEN_PARAMS_JSON: &str = include_str!({abs_path:?});\n",
        path.display(),
    );
