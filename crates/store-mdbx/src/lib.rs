...
//! drain and no maintenance window, so a value format change does not block
//! startup: a value carries its own version tag and is decoded through the
//! chain of decoders this binary retains (see [`version`]).
//!
...
pub use version::{
    ...
    UpgradeCtx, VersionedTable, MAX_UPGRADE_DEPTH,
};
