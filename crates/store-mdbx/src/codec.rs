...
pub enum CodecError {
    ...
    MissingVersionTag {
        /// The versioned table whose value was empty.
        schema: &'static str,
    },
...
/// A typed MDBX table: a named sub-database holding one value per key.
///
...
///
/// Every table maps a key to exactly one value: MDBX's `DUP_SORT` is not used,
/// so a `put` on an existing key replaces it and a `delete` removes the single
/// value there is. A table that needs several values under one key holds a
