    ...
    pub fn decode_raw(buf: &[u8]) -> MessageDecodeResult<DecodedEeMessageData> {
        ...
        match msg.ty() {
            ...
            PREDICATE_UPDATE_MSG_TYPE => {
                ...
                let key_bytes: VarVec<u8, { MAX_PREDICATE_KEY_BYTES }> =
                    decode_codec_msg_body(body)?;
                let new_key = PredicateKeyBuf::try_from(key_bytes.inner().as_slice())
...
mod tests {
    ...
    /// type `0x20` with the codec-encoded new key bytes as body.
    #[test]
