...
//!
//! Each shipped version is an ordinary struct implementing [`SchemaVersion`];
//! the converters between consecutive versions are ordinary [`UpConvert`] impls.
//! [`versioned_value!`](crate::versioned_value) binds them into a chain and
//! names the highest version as current:
//!
//! ```
...
/// One shipped on-disk version of a versioned value.
///
...
/// Folds a shipped version all the way up to the family's current type.
///
...
pub trait LiftToCurrent<Current>: Sized {
...
/// A value stored as a version tag followed by that version's payload.
///
/// Implemented for the family's *current* type by
/// [`versioned_value!`](crate::versioned_value).
pub trait VersionedValue: Sized {
    ...
    fn decode_tagged(bytes: &[u8], ctx: &UpgradeCtx<'_>) -> Result<Self, CodecError>;

    /// Encodes to the current version, tagged.
    fn encode_tagged(&self) -> Result<Vec<u8>, CodecError>;
}
...
pub mod fixtures {
    ...
    /// Checks that `fixtures` cover every version of `V` exactly once, and that
    /// each one decodes and folds up to the current version.
    ///
    ...
    pub fn check_fixtures<V: VersionedValue>(
        fixtures: &[GoldenFixture<'_>],
        ctx: &UpgradeCtx<'_>,
    ) -> Result<Vec<V>, FixtureError> {
        let family = V::FAMILY;

        for fixture in fixtures {
            if !V::VERSIONS.contains(&fixture.version) {
        ...
        fixtures
            .iter()
            .map(|fixture| {
                V::decode_tagged(fixture.bytes, ctx).map_err(|source| FixtureError::Decode {
                    family,
                    version: fixture.version,
                    source,
                })
